...
function OrderSummary({ data, userInput }: orderSummaryProps) {
  ...
  const nextStatus: any =
    data.status! === SD_Status.CONFIRMED
      ? { color: "info", value: SD_Status.IN_PROGRESS }
      : data.status! === SD_Status.IN_PROGRESS
      ? { color: "warning", value: SD_Status.APPROVED }
      : data.status! === SD_Status.APPROVED && {
          color: "success",
      ...
      {!loading && (
        <>
          ...
          <div className="mt-3">
            ...
            <div className="border py-3 px-2">
              <h4 className="text-success">Insurances</h4>
              <div className="p-3">
                {data.cartItems?.map(
                    ...
                    return (
                      <div className="d-flex" key={index}>
                        <div className="d-flex w-100 justify-content-between">
                          <p>{cartItem.insuranceType?.name}</p>
                          <p>
                            ${cartItem.insuranceType?.price} x {cartItem.quantity} =
                          </p>
                        </div>
                        <p style={{ width: "70px", textAlign: "right" }}>
                          $
                          {(cartItem.insuranceType?.price ?? 0) *
                            (cartItem.quantity ?? 0)}
                        </p>
                      </div>
                    );
